  ...
  async function changeFieldCodes() {
    try {
      ...
      if (previewFields) {
        ...
        console.log(
          "[Kintone Dev Tools] Production field codes:",
          productionFieldCodes,
        );
        console.log(
          "[Kintone Dev Tools] Preview field codes:",
          previewFieldCodes,
        );

        // フィールドコードが異なる場合をチェック
        const differentFields = productionFieldCodes.filter(
          (code) => !previewFieldCodes.includes(code),
        );
        const newFields = previewFieldCodes.filter(
          (code) => !productionFieldCodes.includes(code),
        );

        if (differentFields.length > 0 || newFields.length > 0) {
          const warningMessage =
            `運用環境とプレビュー環境でフィールドコードが異なっています。\n\n` +
      ...
      for (const [currentCode, field] of Object.entries(fields)) {
        ...
        if (
          systemFields.includes(field.label) ||
          systemFields.includes(currentCode)
        ) {
          continue;
        }
      ...
      // デバッグ用：フィールドコードの確認
      console.log(
        "[Kintone Dev Tools] Available field codes:",
        Object.keys(fields),
      );
      ...
      function validateFieldCodes(updatedChanges: any[], allChanges: any[]) {
        ...
        for (const change of updatedChanges) {
          ...
          if (forbiddenStrings.includes(newCode)) {
            errors.push(
              `「${label}」: フィールドコードに設定できない文字列です (${newCode})`,
            );
          }
          ...
          if (invalidCharPattern.test(newCode)) {
            errors.push(
              `「${label}」: フィールドコードに使用できない文字が含まれています (${newCode})`,
            );
          }
          ...
          if (startsWithNumberPattern.test(newCode)) {
            errors.push(
              `「${label}」: フィールドコードは半角数字で始めることはできません (${newCode})`,
            );
          }
          ...
          if (newCode !== oldCode) {
            if (usedCodes.has(newCode)) {
              errors.push(
                `「${label}」: フィールドコードが重複しています (${newCode})`,
              );
            }
            if (existingCodes.has(newCode)) {
              errors.push(
                `「${label}」: 既存のフィールドコードと重複しています (${newCode})`,
              );
            }
          }
      ...
      const executeFieldCodeChanges = async (changesToExecute: any[]) => {
        try {
          ...
          for (const change of changesToExecute) {
            console.log(
              `[Kintone Dev Tools] Processing change: ${change.oldCode} → ${change.newCode}`,
            );
          ...
          try {
            ...
            console.log(
              "[Kintone Dev Tools] Preview update response:",
              updateResponse,
            );
            console.log(
              `[Kintone Dev Tools] Successfully updated ${changes.length} field(s) in preview environment`,
            );
            ...
            console.log(
              "[Kintone Dev Tools] Preview environment fields after update:",
              previewCheckResponse.properties,
            );
          } catch (previewError) {
            console.error(
              "[Kintone Dev Tools] Failed to update preview environment:",
              previewError,
            );
            throw previewError;
          }
          ...
          if (confirm(deployConfirmMessage)) {
            ...
            alert(
              `${changesToExecute.length}個のフィールドコードを変更し、運用環境に反映しました。\nページを再読み込みしてください。`,
            );
          } else {
            alert(
              `${changesToExecute.length}個のフィールドコードをプレビュー環境で変更しました。\n運用環境への反映は管理画面から行ってください。`,
            );
          }
          ...
          console.error(
            "[Kintone Dev Tools] Failed to change field codes:",
            error,
          );
